...
- February 12, 2026: Added reliability improvements for market research and existing solutions steps. Backend search service now tracks dataSource ('search-api', 'ai-generated', 'fallback') to indicate where information came from. Frontend shows contextual disclaimers: amber warning for AI-generated data, blue notice for web search data, gray info for fallback frameworks. Citation links with external URLs displayed when search API results are available. "Directional Estimate" and "Verify Independently" labels added to AI-generated market sizes, trends, and competitor data. Supports SerpAPI and Bing Search API keys for real web search results.
```
